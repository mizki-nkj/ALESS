// ============================================================================
// PHASE 4 SAFE CODE
// Summer Vegetation Statistics - Temporal Change Focus
//
// Outputs based on NDVI_mean and NDMI_mean:
//
// NDVI_diff_from_2003
// NDVI_diff_from_2010
// NDVI_diff_from_2011
// NDVI_year_to_year_change
// NDVI_inside_minus_outside
// NDVI_slope_2003_2025
// NDVI_slope_2011_2025
//
// NDMI_diff_from_2003
// NDMI_diff_from_2010
// NDMI_diff_from_2011
// NDMI_year_to_year_change
// NDMI_inside_minus_outside
// NDMI_slope_2003_2025
// NDMI_slope_2011_2025
//
// Target:
// Fukushima, DTRZ + 30 km buffer
// Landsat C2 L2 SR
// 2003-2025 Summer only
//
// Safety design:
// - NDVI and NDMI only.
// - Mean values only as temporal baseline.
// - One band is reduced at a time.
// - Temporal calculations are done on FeatureCollection values,
//   not on raster stacks.
// - Uses SCALE = 30 and TILE_SCALE = 8 based on your successful runs.
// ============================================================================
...
// Confirmed successful setting from your previous phases.
var SCALE = 30;
var TILE_SCALE = 8;
...
// DTRZ + 30 km buffer, limited to Fukushima.
...
// ---------------------------------------------------------------------------
// 6. ONE-BAND GROUPED MEAN
// ---------------------------------------------------------------------------

function groupedMeanForOneBand(image, bandName) {
...
// ---------------------------------------------------------------------------
// 7. BASE YEARLY FEATURE COLLECTION
// NDVI_mean and NDMI_mean by Year x ClassZone.
// ---------------------------------------------------------------------------

function baseFeaturesForYear(year) {
  year = ee.Number(year);

  var image = makeSummerComposite(year);

  var ndviMeanDict = groupedMeanForOneBand(image, 'NDVI');
  var ndmiMeanDict = groupedMeanForOneBand(image, 'NDMI');

  return ee.FeatureCollection(classZoneCodes.map(function(code) {
    code = ee.Number(code);
    var codeStr = code.format();

    var ndviStats = safeGetDict(ndviMeanDict, codeStr);
    var ndmiStats = safeGetDict(ndmiMeanDict, codeStr);
    ...
    return ee.Feature(null, {
      'Year': year,
      'Season': 'Summer',
      'Analysis': 'Research1_Phase4_Temporal_Change',
      ...
      'NDVI_mean': safeGetNumber(ndviStats, 'mean', null),
      'NDMI_mean': safeGetNumber(ndmiStats, 'mean', null)
    });
  }));
}
...
var baseFC = ee.FeatureCollection(
  years.map(function(year) {
    return baseFeaturesForYear(year);
  })
).flatten();

print('Base Phase 4 annual means preview', baseFC.limit(20));


// ---------------------------------------------------------------------------
// 8. TEMPORAL LOOKUP HELPERS
// ---------------------------------------------------------------------------

function getFeatureByCodeYear(fc, code, year) {
  return ee.Feature(
    fc
...
function getValueByCodeYear(fc, code, year, propertyName) {
  var feature = getFeatureByCodeYear(fc, code, year);

  return ee.Algorithms.If(
    feature.propertyNames().contains(propertyName),
    feature.get(propertyName),
    null
  );
}

function slopeForCode(fc, code, startYear, propertyName) {
  var sub = fc
    .filter(ee.Filter.eq('ClassZone_Code', code))
    .filter(ee.Filter.gte('Year', startYear))
    .filter(ee.Filter.notNull(['Year', propertyName]));

  var fit = sub.reduceColumns({
    reducer: ee.Reducer.linearFit(),
    selectors: ['Year', propertyName]
  });

  return ee.Algorithms.If(
    ee.Dictionary(fit).keys().contains('scale'),
    fit.get('scale'),
    null
  );
}
...
// ---------------------------------------------------------------------------
// 9. ADD PHASE 4 TEMPORAL METRICS
// ---------------------------------------------------------------------------

var phase4FC = baseFC.map(function(feature) {
  feature = ee.Feature(feature);

  var year = ee.Number(feature.get('Year'));
  var code = ee.Number(feature.get('ClassZone_Code'));
  var classId = code.mod(100);

  var ndvi = feature.get('NDVI_mean');
  var ndmi = feature.get('NDMI_mean');

  // -------------------------------------------------------------------------
  // 9.1 Differences from base years
  // -------------------------------------------------------------------------

  var ndvi2003 = getValueByCodeYear(baseFC, code, 2003, 'NDVI_mean');
  var ndvi2010 = getValueByCodeYear(baseFC, code, 2010, 'NDVI_mean');
  var ndvi2011 = getValueByCodeYear(baseFC, code, 2011, 'NDVI_mean');

  var ndmi2003 = getValueByCodeYear(baseFC, code, 2003, 'NDMI_mean');
  var ndmi2010 = getValueByCodeYear(baseFC, code, 2010, 'NDMI_mean');
  var ndmi2011 = getValueByCodeYear(baseFC, code, 2011, 'NDMI_mean');

  // -------------------------------------------------------------------------
  // 9.2 Year-to-year change
  // 2003 has no previous year, so it becomes null.
  // -------------------------------------------------------------------------

  var previousYear = year.subtract(1);

  var ndviPrev = ee.Algorithms.If(
    year.gt(START_YEAR),
    getValueByCodeYear(baseFC, code, previousYear, 'NDVI_mean'),
    null
  );

  var ndmiPrev = ee.Algorithms.If(
    year.gt(START_YEAR),
    getValueByCodeYear(baseFC, code, previousYear, 'NDMI_mean'),
    null
  );

  // -------------------------------------------------------------------------
  // 9.3 Inside - Outside difference
  // Same Year and same Land_Class.
  // This output is repeated on both Inside and Outside rows for convenience.
  // -------------------------------------------------------------------------

  var insideCode = ee.Number(100).add(classId);
  var outsideCode = ee.Number(200).add(classId);

  var ndviInside = getValueByCodeYear(baseFC, insideCode, year, 'NDVI_mean');
  var ndviOutside = getValueByCodeYear(baseFC, outsideCode, year, 'NDVI_mean');

  var ndmiInside = getValueByCodeYear(baseFC, insideCode, year, 'NDMI_mean');
  var ndmiOutside = getValueByCodeYear(baseFC, outsideCode, year, 'NDMI_mean');

  // -------------------------------------------------------------------------
  // 9.4 Linear slopes
  // Unit: index value per year.
  // -------------------------------------------------------------------------

  var ndviSlope2003 = slopeForCode(baseFC, code, 2003, 'NDVI_mean');
  var ndviSlope2011 = slopeForCode(baseFC, code, 2011, 'NDVI_mean');

  var ndmiSlope2003 = slopeForCode(baseFC, code, 2003, 'NDMI_mean');
  var ndmiSlope2011 = slopeForCode(baseFC, code, 2011, 'NDMI_mean');

  return feature.set({
    // Differences from baseline years
    'NDVI_diff_from_2003': safeSubtract(ndvi, ndvi2003),
    'NDVI_diff_from_2010': safeSubtract(ndvi, ndvi2010),
    'NDVI_diff_from_2011': safeSubtract(ndvi, ndvi2011),

    'NDMI_diff_from_2003': safeSubtract(ndmi, ndmi2003),
    'NDMI_diff_from_2010': safeSubtract(ndmi, ndmi2010),
    'NDMI_diff_from_2011': safeSubtract(ndmi, ndmi2011),

    // Year-to-year changes
    'NDVI_year_to_year_change': safeSubtract(ndvi, ndviPrev),
    'NDMI_year_to_year_change': safeSubtract(ndmi, ndmiPrev),

    // Inside - Outside differences
    'NDVI_inside_minus_outside': safeSubtract(ndviInside, ndviOutside),
    'NDMI_inside_minus_outside': safeSubtract(ndmiInside, ndmiOutside),

    // Linear trend slopes
    'NDVI_slope_2003_2025': ndviSlope2003,
    'NDVI_slope_2011_2025': ndviSlope2011,

    'NDMI_slope_2003_2025': ndmiSlope2003,
    'NDMI_slope_2011_2025': ndmiSlope2011
  });
});

print('Phase 4 output preview', phase4FC.limit(20));
...
Export.table.toDrive({
  collection: phase4FC,
  description: 'Summer_Vegetation_Statistics_Phase4',
  folder: EXPORT_FOLDER,
  fileNamePrefix: 'Summer_Vegetation_Statistics_Phase4',
  fileFormat: 'CSV',
  selectors: [
    'Year',
    'Season',
...
print('Export prepared: Summer_Vegetation_Statistics_Phase4');
