// ============================================================================
// PHASE 3 SAFE CODE
// Summer Vegetation Statistics - EVI and NBR Focus
//
// Outputs:
// EVI_mean
// EVI_median
...
// - Avoids Reducer.group groupField out of range.
...
// Confirmed successful setting from your Phase 1 / Phase 2 runs.
var SCALE = 30;
var TILE_SCALE = 8;
...
// ---------------------------------------------------------------------------
// 6. ONE-BAND GROUPED STATS
// ---------------------------------------------------------------------------

// For EVI:
// mean, median, p25, p75
var eviReducer = ee.Reducer.mean()
  .combine({
...
// For NBR:
// mean, median only
var nbrReducer = ee.Reducer.mean()
  .combine({
    reducer2: ee.Reducer.median(),
    sharedInputs: true
  });
...
// ---------------------------------------------------------------------------
// 7. PHASE 3 FEATURES BY YEAR
// ---------------------------------------------------------------------------

function phase3FeaturesForYear(year) {
  year = ee.Number(year);

  var image = makeSummerComposite(year);

  var eviStatsDict = groupedStatsForOneBand(image, 'EVI', eviReducer);
  var nbrStatsDict = groupedStatsForOneBand(image, 'NBR', nbrReducer);
  ...
  return ee.FeatureCollection(classZoneCodes.map(function(code) {
    ...
    var eviStats = safeGetDict(eviStatsDict, codeStr);
    var nbrStats = safeGetDict(nbrStatsDict, codeStr);

    return ee.Feature(null, {
      'Year': year,
      'Season': 'Summer',
      'Analysis': 'Research1_Phase3_EVI_NBR',
      ...
      'EVI_mean': safeGet(eviStats, 'mean'),
      'EVI_median': safeGet(eviStats, 'median'),
      'EVI_p25': safeGet(eviStats, 'p25'),
      'EVI_p75': safeGet(eviStats, 'p75'),

      'NBR_mean': safeGet(nbrStats, 'mean'),
      'NBR_median': safeGet(nbrStats, 'median')
    });
  }));
}
...
// ---------------------------------------------------------------------------
// 8. BUILD FEATURE COLLECTION
// ---------------------------------------------------------------------------

var years = ee.List.sequence(START_YEAR, END_YEAR);

var phase3FC = ee.FeatureCollection(
  years.map(function(year) {
    return phase3FeaturesForYear(year);
  })
).flatten();

print('Phase 3 output preview', phase3FC.limit(20));
...
Export.table.toDrive({
  collection: phase3FC,
  description: 'Summer_Vegetation_Statistics_Phase3',
  folder: EXPORT_FOLDER,
  fileNamePrefix: 'Summer_Vegetation_Statistics_Phase3',
  fileFormat: 'CSV',
  selectors: [
    'Year',
    'Season',
...
print('Export prepared: Summer_Vegetation_Statistics_Phase3');
